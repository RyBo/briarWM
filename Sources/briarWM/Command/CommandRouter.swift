...
final class CommandRouter {
    ...
    func perform(_ action: Action) {
        ...
        case .workspaceNext:          manager.switchToDesktopRelative(1)
        case .workspacePrev:          manager.switchToDesktopRelative(-1)
        case .workspaceBack:          manager.switchToDesktopBack()
        case .moveToWorkspace(let n): manager.moveFocusedToDesktop(n)
        case .gapsAdjust(let side, let delta): manager.adjustGaps(side, by: delta)
        case .gapsReset:              manager.resetGaps()
