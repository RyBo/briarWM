...
@Suite struct TabReconcileTests {
    ...
    @Test func tabDecisionCases() {
        ...
        #expect(tabDecision(frameMatch: nil, isFront: true, leafAlreadyUsed: false, bothOnscreen: false) == .adopt)
        #expect(tabDecision(frameMatch: nil, isFront: false, leafAlreadyUsed: false, bothOnscreen: false) == .adopt)
        // Stacked on a tile, one member on-screen → a native tab: front takes over the leaf,
        // background tab is skipped.
        #expect(tabDecision(frameMatch: WinID(3), isFront: true, leafAlreadyUsed: false, bothOnscreen: false) == .rebind(WinID(3)))
        #expect(tabDecision(frameMatch: WinID(3), isFront: false, leafAlreadyUsed: false, bothOnscreen: false) == .ignore)
        // Front tab but its leaf was already claimed this pass → ignore (don't double-rebind).
        #expect(tabDecision(frameMatch: WinID(3), isFront: true, leafAlreadyUsed: true, bothOnscreen: false) == .ignore)
    }

    @Test func stackedButBothOnscreenIsIndependent() {
        // Candidate and matched leaf both on-screen at once → not a tab group (a real group
        // shows one member at a time) but two independent windows sharing a frame — adopt,
        // whatever the front/claimed state (e.g. Ghostty opens new windows at the exact frame).
        #expect(tabDecision(frameMatch: WinID(3), isFront: true, leafAlreadyUsed: false, bothOnscreen: true) == .adopt)
        #expect(tabDecision(frameMatch: WinID(3), isFront: false, leafAlreadyUsed: false, bothOnscreen: true) == .adopt)
        #expect(tabDecision(frameMatch: WinID(3), isFront: true, leafAlreadyUsed: true, bothOnscreen: true) == .adopt)
        // No frame match adopts regardless of the flag.
        #expect(tabDecision(frameMatch: nil, isFront: true, leafAlreadyUsed: false, bothOnscreen: true) == .adopt)
    }
